...
const float real_voltage_reference = 4945.0;
...
float calculate_voltage(){
  float sum = 0.0;
  float voltage_signal = 0.0;
  float sensor_in_delta = 0.0;
  for (int i = 0; i < 4; i++){
    voltage_signal = (analogRead(voltage_sensor_pin) * real_voltage_reference) / 1023.0;
    sum += voltage_signal / voltage_sensitivity;
    delay(250);
  }
  return sum / 4.0;
}
